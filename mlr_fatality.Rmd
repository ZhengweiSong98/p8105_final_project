...
ca_premodel_data = left_join(ca_medical_data, ca_nonmedical_data, by = c("county")) %>%
    ...
    dplyr::select(-fully_vaccinated, -population, -labor_force)
...
ca_model_data = ca_premodel_data %>% 
    ...
    dplyr::select(-test,-land_area_sq_mi,-unemployment,-county)
...
raw_deaths+raw_tests+raw_land+ raw_unemployment+ raw_vaccinated+ raw_labor+ plot_ln_test+ plot_ln_area+ plot_ln_unemployment
...
par(mfrow=c(1,2))
plot(2:7, criterion_plot$cp, xlab="No. of parameters", ylab="Cp Statistic")
abline(0,1)
plot(2:7, criterion_plot$adjr2, xlab="No. of parameters", ylab="Adj R2")
```
...
set.seed(2022)
cv_object <- cv.glmnet(as.matrix(ca_lasso_data[2:7]), ca_lasso_data$death_rate,
...
fit_bestcv <- glmnet(as.matrix(ca_lasso_data[2:7]), ca_model_data$death_rate, lambda = cv_object$lambda.min)
coef(fit_bestcv)
```
...
```{r warning = F, message = F}
mlr_model = lm(death_rate~location+vaccinated+labor_rate+ln_tests+ln_area+ln_unemployment,data = ca_model_data)
summary(mlr_model)
```
...
model_caret = train(death_rate~location+vaccinated+labor_rate+ln_tests+ln_area+ln_unemployment, data = ca_model_data, trControl = train, method = 'lm', na.action = na.pass)
model_caret
