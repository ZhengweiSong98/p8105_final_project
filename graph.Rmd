...
demo_plot = demo %>%
    ...
    mutate(county_name = fct_reorder(county_name, total_cases)) %>% 
    ggplot(aes(x = county_name, y = total_cases, color = county_name)) +
    geom_point(alpha = 1) +
    theme(
...
point <- format_format(big.mark = " ", decimal.mark = ",", scientific = FALSE)
    ...
    summarize(total_cases = sum(reported_cases),
              total_deaths = sum(reported_deaths)) %>% 
    mutate(county_name = fct_reorder(county_name, total_cases)) %>%
    ggplot(aes(x = total_cases, fill = county_name)) +
    geom_histogram(aes(y = ..density..), alpha = 0.5, bins = 50) +
    geom_density(alpha = 0.5, aes(color = county_name)) +
    labs(x = "Total cases",
         y = "Density") +
    theme(
...
demo_worst =
    ...
    summarize(total_cases = sum(reported_cases),
              total_deaths = sum(reported_deaths)) %>%
    select(county_name, total_cases) %>% 
    arrange(desc(total_cases)) 

    head(demo_worst, 10) %>% 
    knitr::kable(
    caption = "Worst countries for each COVID outcome",
...
demo_best =
    ...
    head(demo_best, 10) %>% 
    knitr::kable(
    caption = "Best countries for each COVID outcome",
    col.names = c("County", "Total cases"),
    digits = 2
  )
```
...
demo_worst_p =
    demo_worst %>% 
    filter(total_cases >= 256191) %>% 
    ggplot(aes(x = county_name, y = total_cases, fill = county_name)) +
